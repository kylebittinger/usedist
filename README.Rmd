...
`usedist` includes a convenience function, `pivot_to_numeric_matrix`.
...
`pivot_to_numeric_matrix()`. This function, and the next, require some extra
packages to be installed.  They are listed as suggestions for `usedist`.  If
the aditional packages are not installed on your system, you'll get an error
...
`pivot_to_numeric_matrix()` with `dist_make()`:
